...
const { buildReviewQueues, renderFilters, renderPagination, reviewReturnUrl } = require('./reliabilityReviewQueue');
...
function sourceReviewState(source, today) {
  if (source.section_mode) return 'sections';
  if (!source.pending_version_id) return 'current';
  if (source.pending_comparison?.parse_error) return 'blocked';
  return isPendingVersionEffective(source, today) ? 'ready' : 'future';
}

function renderSources(sources, password, returnField = '') {
  if (!sources.length) {
    return '<p class="empty">No sources match these filters.</p>';
  }
  ...
  const bulkAction = unchangedCandidates.length ? `<form class="bulk-action" method="post" action="/admin/timetable-reliability/sources/bulk-approve-unchanged">
    ${returnField}<input type="hidden" name="candidate_ids" value="${escapeHtml(JSON.stringify(unchangedCandidates.map(candidate => candidate.sourceId)))}">
    ...
    <button class="approve" type="submit">Approve and publish ${unchangedCandidates.length} unchanged route${unchangedCandidates.length === 1 ? '' : 's'} on this page</button>
  </form>` : '';
  ...
  return `${bulkAction}<div class="table-wrap"><table>
    ...
    <tbody>${sources.map((source) => {
      if (source.section_mode) return `<tr><td><strong>${escapeHtml(source.operator)} ${escapeHtml(source.source_key)}</strong><br>${escapeHtml(source.route_name)}</td><td colspan="4">Document evidence · review the individual <a href="#sections">timetable sections</a> below.
...
function renderAudit(audit, samples, password, returnField = '') {
  if (!audit) {
...
function renderAdminPage({ sources, checkRuns, audit, samples, sectionGroups = [], query = {} }, password) {
  const today = johannesburgDate();
  const queues = buildReviewQueues(sources, sectionGroups, query, source => sourceReviewState(source, today), today);
  const { options } = queues;
  const returnField = `<input type="hidden" name="return_query" value="${escapeHtml(new URLSearchParams(options).toString())}">`;
  const sourcePages = renderPagination(queues.sources, 'source_page', options, escapeHtml);
  const sectionPages = renderPagination(queues.sections, 'section_page', options, escapeHtml);
  const operators = [...new Set([...sources.map(source => source.operator), ...(sectionGroups.length ? ['GABS'] : [])])].filter(Boolean).sort();
  ...
  <section aria-label="Review filters">${renderFilters(options, operators, escapeHtml)}
  <p class="muted">Ready candidates appear first by default. Filters apply to both lists; section matches retain all copies for comparison.</p>
  <p><a href="#sources">Sources (${queues.sources.total})</a> · <a href="#sections">Timetable sections (${queues.sections.total})</a> · <a href="#audit">Weekly audit</a></p></section>
  <section id="sources"><h2>Source registry and pending comparisons</h2>${sourcePages}${renderSources(queues.sources.items, password, returnField)}${sourcePages}</section>
  <section id="sections"><h2>Golden Arrow timetable sections</h2>${sectionPages}
  ${renderSectionReviews(queues.sections.items, { escapeHtml, actionToken, comparisonSummary, returnField, today }, password) || '<p class="empty">No timetable sections match these filters.</p>'}${sectionPages}</section>
  <section id="audit"><h2>Weekly source-accuracy audit</h2><p>This compares Fika entries with captured operator PDFs. It is not a punctuality audit.</p>${renderAudit(audit, samples, password, returnField)}</section>
  </main></body></html>`;
}
...
function createTimetableReliabilityHandlers({ database, username, password }) {
  ...
  async function handleSectionAction(req, res, withdraw) {
    ...
    try {
      ...
      res.redirect(303, reviewReturnUrl(req.body.return_query, 'sections'));
  ...
  return {
    ...
    async approve(req, res) {
      ...
      try {
        ...
        res.redirect(303, reviewReturnUrl(req.body.return_query));
      } catch (error) {
    ...
    async bulkApproveUnchanged(req, res) {
      ...
      try {
        ...
        if (req.body.candidate_ids !== undefined) {
          let ids;
          try { ids = JSON.parse(req.body.candidate_ids); } catch { ids = null; }
          if (!Array.isArray(ids) || !ids.length || ids.length > 50 ||
              ids.some(id => !Number.isSafeInteger(id) || id < 1) || new Set(ids).size !== ids.length) {
            return res.status(400).type('text/plain').send('Invalid bulk approval selection. Reload the review page.');
          }
          selectedRows = rows.filter(row => ids.includes(Number(row.id)));
          if (selectedRows.length !== ids.length) {
            return res.status(409).type('text/plain').send('The selected candidates changed. Reload the review page.');
          }
        }
        const candidates = getBulkUnchangedCandidates(selectedRows);
    ...
    async audit(req, res) {
      ...
      try {
        ...
        res.redirect(303, reviewReturnUrl(req.body.return_query, 'audit'));
      } catch (error) {
