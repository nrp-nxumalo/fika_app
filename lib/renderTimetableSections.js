function renderSectionReviews(groups, { escapeHtml: e, actionToken, comparisonSummary }, password) {
  if (!groups.length) return '';
  return `<section><h2>Golden Arrow timetable sections</h2><p>Each timetable number is reviewed independently. Identical copies share a change alert. Source conflicts apply only to overlapping service days.</p>${groups.map(group => `
      ...
      ${variant.copies.map(copy => {
        ...
        return `<div class="section-copy">
          ...
          ${copy.pending_version_id && !copy.parse_error ? `<form method="post" action="/admin/timetable-reliability/sections/${Number(copy.id)}/approve">
            ...
            <button class="approve" type="submit">Approve copy and publish</button></form>` : ''}
